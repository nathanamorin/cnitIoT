...
var door_data_url = "doordata1.csv";
...
function run()
{
  ...
  $("#ex6SliderVal").text(groupByDate[index].key);

  renderBarChart("bar_chart_count_group_by_action", d3.nest().key(function(d){ return d.Action}).entries(groupByDate[index].values), [0,1000]);

  renderPieChart("pie_chart_count_group_by_action",d3.nest().key(function(d){ return d.Action}).entries(groupByDate[index].values));
});
